...
- Error meanings follow WASI and POSIX categories where practical while keeping Facet numeric values.
...
- Facet allocates no mandatory writable filesystem.
...
- Facet 0.1 has no per-child slice descriptor for nested GC scatter/gather.
- Facet imported calls are synchronous.
- The runtime cannot retain borrowed guest linear-memory or GC storage after return.
- Nonblocking I/O uses `ERR_AGAIN` and `facet-poll`.
- Concurrency and scheduler policy remain outside the ABI.
- Facet uses one global `abi_version()`.
...
- Facet does not silently substitute U+FFFD for unrepresentable external text.
